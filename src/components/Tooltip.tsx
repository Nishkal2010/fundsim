...
export function Tooltip({ term, definition, children }: TooltipProps) {
  ...
  function handleClick() {
    window.dispatchEvent(new CustomEvent("finfox:open-term", { detail: term }));
  }
  ...
  return (
    ...
    >
      <span
        className="border-b border-dashed border-[#6366F1] cursor-pointer"
        onClick={handleClick}
        title="Click to ask FinFox"
      >
        {children}
      </span>
      {visible && (
        ...
        >
          <div className="text-xs font-semibold text-[#818CF8] mb-1">
            {term}
          </div>
