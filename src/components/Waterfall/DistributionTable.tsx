...
export function DistributionTable({ data, inputs }: DistributionTableProps) {
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="text-sm font-semibold text-[#F9FAFB]">LP vs GP Summary</h3>
      </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr style={{ borderBottom: '1px solid #374151' }}>
              ...
              <th className="px-5 py-3 text-right text-[10px] uppercase tracking-wider font-semibold" style={{ color: '#10B981' }}>
                Limited Partners
              </th>
              <th className="px-5 py-3 text-right text-[10px] uppercase tracking-wider font-semibold" style={{ color: '#818CF8' }}>
                General Partner
              </th>
            </tr>
          </thead>
          <tbody>
            {[
              ...
              {
                label: 'Profit',
                lp: formatMillions(Math.max(0, lpProfit)),
                gp: formatMillions(Math.max(0, gpProfit)),
              },
              {
                label: 'Net Multiple',
              ...
              },
              {
                ...
                lp: (data.totalLP + data.totalGP) > 0 ? formatPercent(data.totalLP / (data.totalLP + data.totalGP), 1) : '—',
                gp: (data.totalLP + data.totalGP) > 0 ? formatPercent(data.totalGP / (data.totalLP + data.totalGP), 1) : '—',
              },
              {
              ...
              >
                ...
                <td className="px-5 py-2.5 text-right tabular-nums" style={{ color: '#10B981' }}>{row.lp}</td>
                <td className="px-5 py-2.5 text-right tabular-nums" style={{ color: '#818CF8' }}>{row.gp}</td>
              </tr>
            ))}
          </tbody>
        </table>
