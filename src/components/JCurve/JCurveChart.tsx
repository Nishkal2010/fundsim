      ...
      >
        ...
        {payload.map(p => (
          <div key={p.name} style={{ color: p.color }} className="flex justify-between gap-4">
            <span>{p.name}</span>
            <span className="tabular-nums font-medium">{p.value?.toFixed(1)}%</span>
          </div>
        ))}
      </div>
    );
...
export function JCurveChart({ points, scenarioPoints, breakevenYear, troughYear }: JCurveChartProps) {
  const allData = points.map((p, i) => ({
    year: p.year,
    'Net Cash Flow': parseFloat(p.netCashFlow.toFixed(2)),
    'NAV + Net': parseFloat(p.nav.toFixed(2)),
    ...(scenarioPoints ? { 'Scenario B': parseFloat((scenarioPoints[i]?.netCashFlow ?? 0).toFixed(2)) } : {}),
  }));

  const minValue = Math.min(...allData.map(d => Math.min(d['Net Cash Flow'], d['NAV + Net']))) - 5;
  const maxValue = Math.max(...allData.map(d => Math.max(d['Net Cash Flow'], d['NAV + Net']))) + 5;
  ...
  return (
    ...
    >
      <div className="mb-4">
        <h3 className="text-sm font-semibold text-[#F9FAFB]">J-Curve: Net Cash Flow & NAV</h3>
        <p className="text-xs text-[#6B7280] mt-0.5">As % of committed capital</p>
      </div>
      <ResponsiveContainer width="100%" height={360}>
        <ComposedChart data={allData} margin={{ top: 10, right: 20, left: 10, bottom: 5 }}>
          ...
          <ReferenceLine
            ...
            stroke="#374151"
            strokeWidth={2}
            label={{ value: '0%', position: 'right', fill: '#6B7280', fontSize: 10 }}
          />
          ...
          <ReferenceLine
            ...
            strokeDasharray="4 4"
            strokeWidth={1.5}
            label={{ value: 'Trough', position: 'top', fill: '#EF4444', fontSize: 10 }}
          />
          ...
          {breakevenYear && (
            <ReferenceLine
              ...
              strokeDasharray="4 4"
              strokeWidth={1.5}
              label={{ value: 'Breakeven', position: 'top', fill: '#10B981', fontSize: 10 }}
            />
          )}
