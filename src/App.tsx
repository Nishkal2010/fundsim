    ...
    fetch('/api/auth/me', { credentials: 'include' })
      ...
      .then(data => {
        if (data.user) {
          ...
          if (stored) {
            try { setUser(JSON.parse(stored)); } catch { localStorage.removeItem('fundsim_auth'); }
          }
        }
      })
      .catch(() => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#0A0F1C' }}>
        <motion.div
          ...
          style={{ width: 32, height: 32, border: '2px solid #374151', borderTopColor: '#6366F1', borderRadius: '50%' }}
        />
      </div>
    );
  }
